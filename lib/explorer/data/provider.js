...
import { createGfcReferenceStore } from './gfc.js';
...
export function createDataProvider(config) {
  ...
  const gfc = createGfcReferenceStore(config);
  ...
  function references() {
    const registry = gfc.loadRegistry();
    return {
      source: 'gfc_registry',
      sourceLabel: 'GFC Base Sepolia Registry',
      registryId: registry.registryId,
      registryUpdatedAt: registry.registryUpdatedAt,
      network: registry.network,
      projectStatus: registry.projectStatus,
      references: gfc.list(),
  ...
  function reference(value) {
    return gfc.detail(value);
  }

  async function address(value) {
    const normalized = normalizeAddress(value);
    const knownReference = gfc.normalizeKnownAddress(normalized);
    try {
      const live = await rpc.getAddressContext(normalized);
      return {
        ...live,
        network: NETWORK.name,
        chainId: NETWORK.chainId,
        knownGfcReference: Boolean(knownReference),
        gfcReference: knownReference,
      };
    } catch (error) {
      if (!knownReference) throw error;
      return {
        address: normalized,
        kind: 'unknown',
        codePresent: null,
        ...
        warning: 'Live Base Sepolia address classification is currently unavailable. The published GFC reference remains available and no live state is inferred.',
        network: NETWORK.name,
        chainId: NETWORK.chainId,
        knownGfcReference: true,
        gfcReference: knownReference,
      };
    }
  }

  async function gfcSnapshot() {
    const treasury = gfc.find('gfc-test-treasury');
    const presale = gfc.find('gfc-test-presale');
    const live = await rpc.getGfcSnapshot({
    ...
    return {
      ...
      registryUpdatedAt: registry.registryUpdatedAt,
      boundaries: {
        officialMainnetContractsPublished: Boolean(registry.projectStatus?.officialMainnetContractsPublished),
        activePublicMainnetPresale: Boolean(registry.projectStatus?.activePublicMainnetPresale),
        externalAuditReportsPublished: Boolean(registry.projectStatus?.externalAuditReportsPublished),
      },
    };
  }
