...
export function createRpcClient(config) {
  ...
  function decodeAddressResult(value) {
    if (typeof value !== 'string' || !/^0x[0-9a-fA-F]{64}$/u.test(value)) return null;
    const address = `0x${value.slice(-40)}`.toLowerCase();
    if (!isAddress(address) || /^0x0{40}$/u.test(address)) return null;
    return address;
  }

  async function safeRpcRead(operation) {
    try {
  ...
  async function getAddressContext(address) {
    const normalized = normalizeAddress(address);
    const latestBlockHex = await call('eth_blockNumber');
    const [code, nativeBalance] = await Promise.all([
      call('eth_getCode', [normalized, 'latest']),
      safeRpcRead(() => call('eth_getBalance', [normalized, 'latest'])),
    ]);

    const hasCode = typeof code === 'string' && !/^0x0*$/iu.test(code);
    return {
      address: normalized,
      kind: hasCode ? 'contract' : 'wallet',
      codePresent: hasCode,
      nativeBalanceWei: typeof nativeBalance === 'string' ? hexToBigInt(nativeBalance).toString() : null,
  ...
  async function getGfcSnapshot({ treasuryAddress, presaleAddress } = {}) {
    const latestBlockHex = await call('eth_blockNumber');

    const ethCall = (to, data) => call('eth_call', [{ to, data }, 'latest']);
    const tokenAddress = NETWORK.contract;
    const paddedAddress = treasuryAddress && isAddress(treasuryAddress)
      ? treasuryAddress.slice(2).toLowerCase().padStart(64, '0')
      : null;

    const [totalSupplyHex, ownerHex, pendingOwnerHex, treasuryTokenHex, treasuryNativeHex, presaleCode] = await Promise.all([
      safeRpcRead(() => ethCall(tokenAddress, '0x18160ddd')),
      safeRpcRead(() => ethCall(tokenAddress, '0x8da5cb5b')),
      safeRpcRead(() => ethCall(tokenAddress, '0xe30c3978')),
      paddedAddress ? safeRpcRead(() => ethCall(tokenAddress, `0x70a08231${paddedAddress}`)) : Promise.resolve(null),
      treasuryAddress && isAddress(treasuryAddress) ? safeRpcRead(() => call('eth_getBalance', [treasuryAddress, 'latest'])) : Promise.resolve(null),
      presaleAddress && isAddress(presaleAddress) ? safeRpcRead(() => call('eth_getCode', [presaleAddress, 'latest'])) : Promise.resolve(null),
    ]);

    const decodeUint = (value) => {
      try { return typeof value === 'string' ? hexToBigInt(value).toString() : null; } catch { return null; }
    };
    const presaleCodePresent = typeof presaleCode === 'string'
      ? !/^0x0*$/iu.test(presaleCode)
      : null;
    ...
    return {
      ...
      token: {
        ...
        totalSupplyBaseUnits: decodeUint(totalSupplyHex),
        owner: decodeAddressResult(ownerHex),
        pendingOwner: decodeAddressResult(pendingOwnerHex),
      },
      treasury: treasuryAddress && isAddress(treasuryAddress) ? {
        address: normalizeAddress(treasuryAddress),
        tokenBalanceBaseUnits: decodeUint(treasuryTokenHex),
        nativeBalanceWei: decodeUint(treasuryNativeHex),
      } : null,
      presale: presaleAddress && isAddress(presaleAddress) ? {
        address: normalizeAddress(presaleAddress),
        codePresent: presaleCodePresent,
      } : null,
    };
  }
  ...
  return { getRecentTransferLogs, getTransaction, getAddressContext, getGfcSnapshot, status };
}
