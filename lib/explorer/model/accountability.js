...
export function buildAccountabilityModel(
  ...
  const executionStatusKnown = successful || reverted;
  const directReceipt = Boolean(transaction.receiptAvailable);
  ...
  const funds = hasGfcTransfer
    ...
    : section(
        ...
        successful
          ...
          : reverted
            ? `${directReceipt ? 'The transaction receipt' : 'The indexed transaction execution record'} reports a reverted transaction, so no completed tGFC transfer is established.`
            : 'No indexed tGFC Transfer event or complete execution evidence is available for this record.',
  ...
  const authority = section(
    'not_verifiable',
    transaction.from
      ? 'The transaction identifies an onchain sender (`from`), but the available data does not establish whether that sender was authorized under GFC governance, policy, or an offchain mandate.'
      : 'The available indexed event evidence does not establish the transaction sender (`from`) or whether any actor was authorized under GFC governance, policy, or an offchain mandate.',
    transaction.from
      ? [
          {
            type: 'transaction_sender',
            label: 'Transaction sender (`from`)',
    ...
    [
      ...
      'An ERC-20 Transfer event sender must not be assumed to be the transaction sender (`from`).',
  ...
  if (successful) {
    outcomes = section(
      'partially_verifiable',
      hasGfcTransfer
        ? `${directReceipt ? 'The transaction receipt' : 'The indexed transaction execution record'} reports successful technical execution, and the displayed tGFC Transfer event establishes the corresponding onchain event. Broader real-world outcomes remain unproven.`
        : `${directReceipt ? 'The transaction receipt' : 'The indexed transaction execution record'} reports successful technical execution, but no broader documented outcome is available in this evidence set.`,
      evidenceItems,
      [
    ...
    outcomes = section(
      'verified',
      `${directReceipt ? 'The transaction receipt' : 'The indexed transaction execution record'} reports that this transaction reverted; no successful state transition is claimed.`,
      evidenceItems,
      [
    ...
    outcomes = section(
      'partially_verifiable',
      'The indexed tGFC Transfer event verifies the displayed onchain event evidence. A direct transaction receipt is not available from the current evidence source, so receipt-dependent execution facts and broader real-world outcomes remain unverified.',
      evidenceItems,
      [
        'The current evidence source does not provide a direct transaction receipt.',
